...
# Get the current script's path as a pathlib.Path object
script_path = pathlib.Path(__file__)
sys.path.append(str(script_path.parent.parent.parent/pathlib.Path("src")))
...
import argparse
from batchMeans import batch_means_conf_interval
...
if __name__ == '__main__':
    ...
    with basic_model:
        ...
        var_names = [var.name for var in basic_model.free_RVs]
        tIdx=var_names.index(f"y{N}")
    ...
    outfile.write(f"{np.round(np.mean(batch_means_result[tIdx]),5)},{np.round(end-start,5)},{np.round(err,5)}\n")
    outfile.close()
